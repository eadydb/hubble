...
func TestCurrentContext(t *testing.T) {
	testutil.Run(t, "valud conetxt", func(t *testutil.T) {
	...
	testutil.Run(t, "kubeconfig CLI flag takes precedence", func(t *testutil.T) {
		resetKubeConfig(t, changeKubeConfig)
		kubeConfig := t.TempFile("config", []byte(changeKubeConfig))
...
func TestGetRestClientConfig(t *testing.T) {
	testutil.Run(t, "valud conetxt", func(t *testutil.T) {
		...
		t.CheckNoError(err)
		t.CheckDeepEqual("https://foo.com", cfg.Clusters["cluster-foo"].Server)
	})
	...
	testutil.Run(t, "kube-config immutability", func(t *testutil.T) {
		...
		if err = os.WriteFile(kubeConfig, []byte(changeKubeConfig), 0o644); err != nil {
			t.Error(err)
		}
